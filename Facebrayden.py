...
def readTrainingLabels():
	# Open The File
	file = open("facedata/facedatatrainlabels" , "r")
...
def readTrainingImages():
	...
	file = open("facedata/facedatatrain", "r")
...
def readTestingLabels():
	# Open The File
	file = open("facedata/facedatatestlabels" , "r")
...
def readTestingImages():
	# Open the Training Images
	file = open("facedata/facedatatest", "r")
	...
	curr_image = zeros((70,60))
	curr_image_original = [[' ' for x in range(60)] for y in range(70)] 
	...
	for line in file:
		...
		j = 0
		curr_image_original.append(line)
		for character in line:
			if line[j] == ' ':
				curr_image[(i)%70][j] = 0
				curr_image_original[(i)%70][j] = ' '
			else:
				curr_image[(i)%70][j] = 1
				curr_image_original[(i)%70][j] = '#'
			j+=1
...
def main():
	...
	for i in xrange(1,2):
		...
		for x in xrange(0, len(testingImagesList)):
			hypotheticalLabels.append(numClassifierFace(classesList,testingImagesList[x], originalTestingImagesList[x]))
